...
namespace Nbt.Services.Scf.CashIn.Validator.CCTalk
{
     ...
     public sealed class CCTCommunicationA
    {

        public const byte feedMasher = 99;
        public const int rotatePlaces = 12;

        //static readonly log4net.ILog _logger = log4net.LogManager.GetLogger(typeof(CCTCommunicationA).Name);
        ...
        private static volatile CCTCommunicationA instance;
        ...
        public static CCTCommunicationA Instance
        {
            get
            {
                if (CCTCommunicationA.instance == null)
                    CCTCommunicationA.instance = new CCTCommunicationA();
                return CCTCommunicationA.instance;
            }
        }

        private CCTCommunicationA()
        {
            Log.Debug("Starting CCTCommunicationA ...");
        ...
        public bool CheckAcceptorPresent(string acceptorType, byte CCTalkAddress, ref bool useCRCChecksum, byte[] encrytionKey)
        {
            ...
            CCTCommunicationA.Instance.SendCCTalkCommand(1, CCTalkAddress, 245, true, bArr1, out flag1, out bArr2, out s1, false, useCRCChecksum, encrytionKey);
        ...
        public bool CheckAcceptorPresentEncrypted(string acceptorType, byte address, ref bool useCRCChecksum, ref bool useEncryption, byte[] encrytionKey)
        {
            ...
            CCTCommunicationA.Instance.SendCCTalkCommand(1, address, 245, true, bArr1, out flag1, out bArr2, out s1, useEncryption, useCRCChecksum, encrytionKey);
            if (bArr2 != null)
            {
                s2 = asciiencoding.GetString(bArr2);
                if (s2 == acceptorType)
            ...
            useCRCChecksum = true;
            useEncryption = false;
            CCTCommunicationA.Instance.SendCCTalkCommand(address, 245, bArr1, timeOut, out bArr2, CCTMessageType.StandardCRC16, encrytionKey);
            ...
            useCRCChecksum = false;
            useEncryption = false;
            CCTCommunicationA.Instance.SendCCTalkCommand(address, 245, bArr1, timeOut, out bArr2, CCTMessageType.StandardCRC8, encrytionKey);
            ...
            useEncryption = true;
            useCRCChecksum = false;
            CCTCommunicationA.Instance.SendCCTalkCommand(address, 245, bArr1, timeOut, out bArr2, CCTMessageType.EncryptedCRC8, encrytionKey);
        ...
        public bool FindAcceptor(string acceptorType, ref byte CCTalkAddress, ref bool useCRCChecksum, byte[] encryptionCode)
        {
            ...
            ASCIIEncoding asciiencoding = new ASCIIEncoding();
            useCRCChecksum = false;
            flag2 = CCTCommunicationA.Instance.SendCCTalkCommand(1, 0, 253, true, bArr1, out flag1, out bArr2, out s1, false, useCRCChecksum, encryptionCode);
            ...
            try
            {
                ...
                if (flag2)
                {
                    ...
                    {
                        byte b1 = bArr4[i1];
                        CCTCommunicationA.Instance.SendCCTalkCommand(1, b1, 245, true, bArr1, out flag1, out bArr2, out s1, false, useCRCChecksum, encryptionCode);
                        s2 = asciiencoding.GetString(bArr2);
                        if (s2 == acceptorType)
                        {
                            CCTalkAddress = b1;
                ...
                useCRCChecksum = true;
                flag2 = CCTCommunicationA.Instance.SendCCTalkCommand(1, 0, 253, true, bArr1, out flag1, out bArr2, out s1, true, useCRCChecksum, encryptionCode);
                ...
                if (flag2)
                {
                    ...
                    {
                        byte b2 = bArr5[i2];
                        CCTCommunicationA.Instance.SendCCTalkCommand(1, b2, 245, true, bArr1, out flag1, out bArr2, out s1, true, useCRCChecksum, encryptionCode);
                        s2 = asciiencoding.GetString(bArr2);
                        if (s2 == acceptorType)
                        {
                            CCTalkAddress = b2;
                ...
                if (flag2)
                {
                    ...
                    {
                        byte b2 = bArr5[i2];
                        CCTCommunicationA.Instance.SendCCTalkCommand(1, b2, 245, true, bArr1, out flag1, out bArr2, out s1, false, useCRCChecksum, encryptionCode);
                        s2 = asciiencoding.GetString(bArr2);
                        if (s2 == acceptorType)
                        {
                            CCTalkAddress = b2;
        ...
        public static string GetVirtualComPort()
        {
            ...
            foreach (CommunicationResource cr in sp)
            {
                if (cr.PortType == ResourceType.COIN_ACCEPTOR_SERIAL_PORT &&
                    cr.ProtocolType == DeviceProtocol.PROTOCOL_CCTALK)
                {
                    return cr.PortName;
                }
            }
